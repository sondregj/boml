...
BOML은 버전 0.5.0부터 매우 안정적인 버전으로 간주합니다. 목표는 버전 1.0.0을 버전 0.5.0과 역호환성(가능한 한 사용자가 확인할 수 있을 정도로)을 가지게 하는 것입니다. 모든 구현은 0.5.0과 호환되도록 강력히 권장되며, 1.0.0으로 간편하게 업그레이드를 진행할 수 있도록 할 것입니다.

Objectives
----------

BOML은 명확한 의미를 갖으며, 읽기 쉬운 최소한의 구성 파일 형식을 목표로 삼고 있습니다. BOML은 해시 테이블에 분명하게 대응되도록 설계되어있습니다. BOML은 다양한 언어로 된 데이터 구조를 쉽게 구문분석할 수 있어야 합니다.
...
* BOML은 대소 문자를 구분합니다.
* BOML 파일은 유효한 UTF-8으로 인코딩된 유니코드 문서여야 합니다.
...
BOML 문서에서 기본 빌딩 블록은 키값 쌍으로 표현합니다.
...
위에 나열되어있지 않은 다른 이스케이프 문자열은 모두 예약되어있으므로 BOML에서 사용하면 오류가 발생합니다.

때로는 텍스트의 구절 (예, 번역 파일)을 표현하거나 아주 긴 문자열을 여러 줄로 나누고 싶을 때가 있습니다. BOML은 이것을 쉽게 처리할 수 있습니다.
...
BOML 파서는 플랫폼에 맞게 개행문자를 정규화할 수 있어야 합니다.
...
Windows 경로나 정규 표현식을 자주 사용한다면, 역슬래시나 이스케이핑하는 일은 금세 실증이 나는 번거러운 일이며 종종 에러의 원인이됩니다. 이것을 도우려고 BOML은 이스케이프를 전혀 허용하지 않는 리터럴 문자열을 지원합니다.
...
이스케이프가 없으므로 작은따옴표로 묶인 리터럴 문자열 안에 작은따옴표를 사용할 방법은 없습니다. 그 문제를 해결하기 위해 BOML에서는 리터럴 여러 줄 표기가 가능합니다.
...
원하지 않는 경우, 모든 상위 테이블(super-tables)을 지정할 필요가 없습니다. BOML은 당신이 그걸 만들지 않아도 알고 있습니다.
...
BOML 파일은 `.boml` 확장자를 사용해야합니다.

MIME Type
---------

BOML 파일을 인터넷을 통해서 전송할 경우, MIME 타입으로 `application/boml`을 사용합니다.

Comparison with Other Formats
-----------------------------

어떤면에서 BOML은 JSON과 매우 유사합니다. 간단하고 명료하며 유비쿼터스 데이터 타입에 쉽게 대응할 수 있습니다. JSON은 주로 컴퓨터 프로그램에서 읽고 쓰는 데이터를 직렬화하는 데 적합합니다. BOML이 JSON과 다른 점은 사람이 읽고 쓰는데 쉽다는 점입니다. 주석이 가장 좋은 예입니다. 한 프로그램에서 다른 프로그램으로 데이터를 전송할 때에는 아무런 도움이 되지 않지만, 손으로 편집할 수 있는 구성 파일에는 매우 유용합니다.

YAML 포맷은 BOML과 같은 구성 파일을 지향합니다. 그러나 많은 목적을 위해 YAML은 지나치게 복잡한 솔루션이 되었습니다. BOML은 단순성을 목표로 합니다만 YAML 스팩에서의 목표는 분명하지 않습니다: http://www.yaml.org/spec/1.2/spec.html
...
다음 카탈로그로 되어있는 [공식 BOML 위키](https://github.com/boml-lang/boml/wiki)가 있습니다:

* BOML을 사용하는 프로젝트
* 구현체 (Implementations)
* 검사기 (Validators)
* BOML 디코더, 인코더용 언어 테스트 제품군
* 에디터 지원
* 인코더
* 컨버터

해당 목록을 보거나 목록을 추가할 수 있습니다. BOML 커뮤니티에 참여해주셔서 감사합니다!
