...
BOML 旨在成为一个语义明显且易于阅读的最小化配置文件格式。
BOML 被设计成可以无歧义地映射为哈希表。
BOML 应该能很容易地被解析成各种语言中的数据结构。
...
* BOML 是大小写敏感的。
* BOML 文件必须是合法的 UTF-8 编码的 Unicode 文档。
...
所有上面未列出的其它转义序列都是保留的，如果被用了，BOML 应当生成一个错误。
...
BOML 解析器可以相对灵活地解析成对所在平台有效的换行字符。
...
为了帮助搞定这点，BOML 支持字面量字符串，它完全不允许转义。
...
万幸，BOML 支持一种多行版本的字面量字符串来解决这个问题。
...
BOML 文件应当使用 `.boml` 扩展名。

MIME 类型
---------

在互联网上传输 BOML 文件时，恰当的 MIME 类型是 `application/boml`。

与其它格式的比较
----------------

某种程度上讲 BOML 与 JSON 非常相似：简单，规格明确，并且宜于转化成普适的数据类型。
JSON 适合序列化由计算机程序读写的数据。
BOML 与 JSON 的不同之处是它的重点在于宜于人类读写。
注释就是一个好例子：当数据在程序之间传送时，它们有些多余，但是在手工编辑的配置文件中非常有帮助。

YAML 格式就像 BOML 一样，是以配置文件为导向的。
然而……因为种种原因，YAML 是一种过于复杂的解决方案。
BOML 旨在简易，这个目标在 YAML 的规范当中不那么明显：http://www.yaml.org/spec/1.2/spec.html
...
我们有一个[官方 BOML Wiki](https://github.com/boml-lang/boml/wiki) 收录了以下内容：

* 使用了 BOML 的项目
* 可用实现清单
* 验证程序
* BOML 解码器和编码器的语言无关测试套件
* 编辑器支持
* 编码器
* 格式转换器
